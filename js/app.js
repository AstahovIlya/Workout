(() => {
    ...
    let valuePropertys = document.querySelectorAll(".number");
    for (let valueProperty of valuePropertys) {
        let start = 0;
        let end = valueProperty.getAttribute("data-end");
        let counter = setInterval((function() {
            if (end != Math.floor(end)) start = (+start + .01).toFixed(2); else start += 1;
            if (valueProperty.classList.contains("plus")) valueProperty.textContent = `${start}+`; else valueProperty.textContent = start;
